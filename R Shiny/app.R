             ...
             tabPanel("Path Finder",
                      sidebarPanel(
                        ...
                        tags$head(tags$style("#predict{height:100vh !important;}")),
                          ...
                          textInput(inputId = "wind", label = "Enter wind"),
...
server<- function(input, output){
  ...
  output$predict<-renderPlotly({
    input$goButton2
    full_input<-reactive({hur[hur$Latitude >= (as.numeric(isolate(input$lat))-1) & hur$Latitude <= (as.numeric(isolate(input$lat))+1) & hur$Longitude >= (as.numeric(isolate(input$lng))-1) & hur$Longitude <= (as.numeric(isolate(input$lng))+1) & hur$Maximum.Wind>=(as.numeric(isolate(input$wind))-5) & hur$Maximum.Wind <= (as.numeric(isolate(input$wind))+5),]})
    full_join<-reactive({arrange(count(group_by(full_input(), ID), Maximum.Wind), desc(Maximum.Wind))[1:5,]})
    full_join2<-reactive({semi_join(hur, full_join(), by = "ID")})
    path.plot <- plot_mapbox(mode = 'scatterbox') %>%
      add_markers(
